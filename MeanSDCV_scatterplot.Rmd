...
### Mean
To understand the average performance of one player, we can simply calculate the mean of these weekly scores, which is the sum of all weekly scores, divided by the number of weeks.  Insert equation.

Each player is unlikely to score the same point every game. While some players score consistently, others may score differently week to week. To measure the variability, we can reply on standard deviation, which is one of commonly used statistics. In this case, standard deviation measures the average of the distance of each weekly score from the mean. As standard deviation is never negative, the larger the value, the more variability the performance.

An associated statistic is coefficient of variation, which describes how much variability compared to the mean. It is defined as standard deviation, divided by mean. Its math expression is: (insert equation) Since it shows the magnitude of variability relative to the average state, it is particularly useful when you compare more than one variable that has different units. 
